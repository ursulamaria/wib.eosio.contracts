 ...
 */
#include <eosio.ttt/eosio.ttt.hpp>
...
struct impl {
   ...
   void on(const create& c) {
      ...
      games existing_host_games("tic.tac.toe"_n, c.host.value);
      auto itr = existing_host_games.find( c.challenger.value );
   ...
   void on(const restart& r) {
      ...
      games existing_host_games("tic.tac.toe"_n, r.host.value);
      auto itr = existing_host_games.find( r.challenger.value);
   ...
   void on(const move& m) {
      ...
      // Check if game exists
      games existing_host_games("tic.tac.toe"_n, m.host.value);
      auto itr = existing_host_games.find( m.challenger.value );
      eosio_assert(itr != existing_host_games.end(), "game doesn't exists");
...
//EOSIO_DISPATCH( tic_tac_toe::impl, (is_empty_cell)(is_valid_movement)(get_winner)(on) )
